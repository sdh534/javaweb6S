...
<script>
    ...
    let imgsrc = '';
   	for(let i=0; i<images.length; i++){
   		imgsrc += images[i].src.substring(images[i].src.indexOf("review/")+7,images[i].src.length) + "/";
   	}
   	console.log(imgsrc);
   	reviewForm.review_photo.value = imgsrc;
		...
		　　　　url: '${ctp}/member/myPage/myReviewUpdate', 
		　　　　data: formData, 
		...
		<div class="container-fluid pt-4 px-0">
			<div class="row">
													...
													<div class="w-100">
														...
														<div class="ms-3 mt-2">
															<div
																...
																<font id="starValue"> <c:if
																		test="${vo.review_rating != null}">${vo.review_rating}</c:if>
																</font>
																<script> $("#star-rating-checked").css('width', ${vo.review_rating}*20+"%");</script>
									...
									<div class="p-4">
										<div class="textarea-container">
											<textarea name="review_content" id="review_content"
												...
												placeholder="최소 10자 이상 입력해주세요."><c:if
													test="${vo.review_content != null}">${vo.review_content}</c:if> </textarea>
											<p id="charCount">0</p>
										</div>
									</div>
												...
												<div class="mb-3">
													...
													<div class="mb-3 d-flex flex-wrap" id="imagesPreview"
														...
														<c:if test="${vo.review_photo !='' }">
															<c:set var="images"
																value="${fn:split(vo.review_photo,'/')}"></c:set>
				...
				<input type="hidden" name="p_idx" value="${vo.p_idx}"> <input
					type="hidden" name="review_photo" value="${vo.review_photo}">
				<input type="hidden" name="review_rating" id="review_rating"
					value="${vo.review_rating}"> <input type="hidden"
					name="review_idx" value="${vo.review_idx}"> <input
...
<script>
 	 	let width=0;
 	 	let clickCheck=1;
 	 	let savewidth = ${vo.review_rating}*20;
