	...
	<section class="py-5">
		<div class="container px-4 px-lg-5 mt-5">
			<div
				...
				<div class="d-flex">
					<font color="red">상품 <b>${totRecCnt}</b>개
					</font>
					<!-- <div class="ml-3">
             <select class="btn p-0 h-100" name="pageNum" style="font-size: 11pt;">
                 <option>보기</option>
			...
			<div class="text-right">
				<div class="pick_list_box">
					<ul class="pick_list ">
						<li class="custom-control custom-radio custom-control-inline ml-3">
							...
							onclick="location.href='${ctp}/product/productList?mainCode=${paging}&sort=low-price&middleCode=${middleCode}'">
						...
						<li class="custom-control custom-radio custom-control-inline ml-3">
							...
							onclick="location.href='${ctp}/product/productList?mainCode=${paging}&sort=high-price&middleCode=${middleCode}'">
						...
						<li class="custom-control custom-radio custom-control-inline ml-3">
							...
							onclick="location.href='${ctp}/product/productList?mainCode=${paging}&sort=review&middleCode=${middleCode}'">
						...
						<li class="custom-control custom-radio custom-control-inline ml-3">
							...
							onclick="location.href='${ctp}/product/productList?mainCode=${paging}&sort=latest&middleCode=${middleCode}'">
				...
				<div class="text-center  p-3">
					<div>
						<button type="button"
							class="btn btn-dark  <c:if test="${middleCode == 'all'}">btn-active</c:if>"
							onclick="location.href='${ctp}/product/productList?mainCode=${paging}&middleCategory=all'">
							전체</button>
						<c:forEach var="middleVO" items="${middleVOS}">
							<button type="button"
								class="btn btn-dark  <c:if test="${middleCode ==  middleVO.c_middleCode}">btn-active</c:if>"
								onclick="location.href='${ctp}/product/productList?mainCode=${paging}&middleCode=${middleVO.c_middleCode}'">
								${middleVO.c_middleName}</button>
						...
						<div class="col mb-3 ">
							<div class="cards pointer pointerBig"
								...
								<img class="card-img-top" width="100" height="100"
									src="${ctp}/data/product/${vo.p_thumbnailIdx}" alt="..." />
								<!-- Product details-->
									...
									<div class="row p-1">
										...
										<div class="col-sm-9 col-xl-9 text-right product-text"
											...
											<font color="#bbb"> <c:if test="${discount > 0 }">
													<del>
														<fmt:formatNumber value="${vo.p_origPrice}"
															pattern="#,###" />
														원
													</del>
												</c:if>
				...
				<ul class="pagination justify-content-center pagination-sm">
					<c:if test="${pageVO.pag > 1}">
						<li class="page-item"><a class="page-link text-secondary"
							href="${ctp}/product/productList?mainCode=${paging}&pageSize=${pageVO.pageSize}&pag=1&sort=${sort}&middleCode=${middleCode}">첫페이지</a></li>
					</c:if>
					<c:if test="${pageVO.curBlock > 0}">
						<li class="page-item"><a class="page-link text-secondary"
							href="${ctp}/product/productList?mainCode=${paging}&pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}&sort=${sort}&middleCode=${middleCode}">이전블록</a></li>
					</c:if>
					<c:forEach var="i" begin="${pageVO.curBlock*pageVO.blockSize + 1}"
						...
						<c:if test="${i <= pageVO.totPage && i != pageVO.pag}">
							<li class="page-item"><a class="page-link text-secondary"
								href="${ctp}/product/productList?mainCode=${paging}&pageSize=${pageVO.pageSize}&pag=${i}&sort=${sort}&middleCode=${middleCode}">${i}</a></li>
						</c:if>
					</c:forEach>
					<c:if test="${pageVO.curBlock < pageVO.lastBlock}">
						<li class="page-item"><a class="page-link text-secondary"
							href="${ctp}/product/productList?mainCode=${paging}&pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock+1)*pageVO.blockSize + 1}&sort=${sort}&middleCode=${middleCode}">다음블록</a></li>
					</c:if>
					<c:if test="${pageVO.pag < pageVO.totPage}">
						<li class="page-item"><a class="page-link text-secondary"
							href="${ctp}/product/productList?mainCode=${paging}&pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}&sort=${sort}&middleCode=${middleCode}">마지막페이지</a></li>
					</c:if>
				</ul>
			</div>
